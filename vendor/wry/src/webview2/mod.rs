...
impl InnerWebView {
        ...
        if let Some(mut url) = attributes.url {
            if let Some((protocol, _)) = url.split_once("://") {
                if custom_protocols.contains(protocol) {
                    ...
                    url = custom_protocol_workaround::apply_uri_work_around(
                        &url,
                        http_or_https,
                        protocol,
                    )
                    ...
                    let features = args
                        ...
                        .map(|f| {
                            ...
                            if has_position.as_bool() {
                                ...
                                let _ = f.Top(&mut top);
                                position
                                    .replace(dpi::LogicalPosition::new(left as f64, top as f64));
                            }
    ...
    unsafe fn load_extensions(webview: &ICoreWebView2, extension_path: &PathBuf) -> Result<()> {
        ...
        for entry in fs::read_dir(extension_path)? {
            ...
            let handler =
                ProfileAddBrowserExtensionCompletedHandler::create(Box::new(|_, _| Ok(())));
...
impl InnerWebView {
    ...
    pub fn bounds(&self) -> Result<Rect> {
        ...
        if self.is_child {
            ...
            bounds.position =
                PhysicalPosition::new(position_point[0].x, position_point[0].y).into();
        } else {
