...
use webkit2gtk::{
    ...
    NetworkProxySettings, NotificationPermissionRequest, PermissionRequestExt,
    ...
    UserMediaPermissionRequest, UserMediaPermissionRequestExt, UserScript, UserScriptInjectionTime,
...
impl InnerWebView {
    ...
    fn cookie_into_soup_cookie(cookie: &cookie::Cookie<'_>) -> soup::Cookie {
        ...
        if let Some(dt) = cookie.expires_datetime() {
            soup_cookie
                .set_expires(&gtk::glib::DateTime::from_unix_utc(dt.unix_timestamp()).unwrap());
        }
    ...
    pub trait CookieManageExt: IsA<CookieManager> + 'static {
        fn all_cookies<P: FnOnce(std::result::Result<Vec<soup::Cookie>, glib::Error>) + 'static>(
            ...
            >(
                ...
                let ret = webkit_cookie_manager_get_all_cookies_finish(
