...
impl<'a> WebViewBuilder<'a> {
    ...
    pub fn build<W: HasWindowHandle>(self, window: &'a W) -> Result<WebView> {
        ...
        InnerWebView::new(window, self.attrs, self.platform_specific)
...
impl WebViewBuilderExtDarwin for WebViewBuilder<'_> {
    ...
    fn with_on_web_content_process_terminate_handler(
        mut self,
...
pub trait WebViewExtDarwin {
    ...
    fn fetch_data_store_identifiers<F: FnOnce(Vec<[u8; 16]>) + Send + 'static>(cb: F)
        -> Result<()>;
