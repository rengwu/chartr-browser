    ...
    fn new_ns_view(
        ...
        unsafe {
            ...
            if (cfg!(target_os = "macos")
                ...
                || (cfg!(target_os = "ios")
                    && (version.0 > 15 || (version.0 == 15 && version.1 >= 4)))
            {
            ...
            let webview = {
                ...
                let webview: Retained<WryWebView> = objc2::msg_send![super(webview), initWithFrame: frame, configuration: &**config];
                    ...
                    if let Some(policy) = attributes.background_throttling {
                        let policy_value = match policy {
                            BackgroundThrottlingPolicy::Disabled => {
                                WKInactiveSchedulingPolicy::None.0
                            }
                            BackgroundThrottlingPolicy::Suspend => {
                                WKInactiveSchedulingPolicy::Suspend.0
                            }
                            BackgroundThrottlingPolicy::Throttle => {
                                WKInactiveSchedulingPolicy::Throttle.0
                            }
                        };
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                webview.setAllowsBackForwardNavigationGestures(
                    attributes.back_forward_navigation_gestures,
                );
            ...
            let proto_navigation_policy_delegate =
                ProtocolObject::from_ref(&*navigation_policy_delegate);
  ...
  value: Object.freeze({postMessage: function(s) {window.webkit.messageHandlers.ipc.postMessage(s);}})
        ...
        if let Some(scripts) = &mut *self.pending_scripts.lock().unwrap() {
            ...
            unsafe {
                ...
                if let Some(callback) = callback {
                    let handler =
                        block2::RcBlock::new(move |val: *mut AnyObject, _err: *mut NSError| {
                            ...
                            if !val.is_null() {
                                ...
                                let json_string = NSString::initWithData_encoding(
                                    json_string,
                                    &json_ns_data,
                    ...
                    let handler: Option<
                        block2::RcBlock<dyn Fn(*mut AnyObject, *mut NSError)>,
                    > = None;
    ...
    unsafe fn cookie_from_wkwebview(cookie: &NSHTTPCookie) -> cookie::Cookie<'static> {
        ...
        let expires = match expires {
            Some(datetime) => cookie::time::OffsetDateTime::from_unix_timestamp(
                datetime.timeIntervalSince1970() as i64,
            )
            .ok()
    ...
    ///
    /// Needs to run on main thread and needs an event loop to run.
    pub fn remove_data_store<F: FnOnce(crate::Result<()>) + Send + 'static>(
        uuid: &[u8; 16],
        cb: F,
    ) {
