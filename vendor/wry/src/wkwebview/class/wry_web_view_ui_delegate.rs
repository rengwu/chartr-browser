            ...
            let decision = if let Some(handler) = &self.ivars().permission_handler {
                ...
                match capture_type {
                    WKMediaCaptureType::Camera => {
                        translate_response(handler(PermissionKind::Camera))
                    }
                    WKMediaCaptureType::Microphone => {
                        translate_response(handler(PermissionKind::Microphone))
                    }
                    WKMediaCaptureType::CameraAndMicrophone => {
                        ...
                        match (mic_res, cam_res) {
                            (PermissionResponse::Allow, PermissionResponse::Allow) => {
            ...
            if let Some(new_window_req_handler) = &self.ivars().new_window_req_handler {
                ...
                match new_window_req_handler(
                    ...
                    NewWindowFeatures {
                        ...
                        position: if let (Some(x), Some(y)) =
                            (window_features.x(), window_features.y())
                        {
                    ...
                    NewWindowResponse::Allow => {
                        ...
                        let delegate = WryNSWindowDelegate::new(
                            mtm,
                            Box::new(move || {
                                new_windows.borrow_mut().retain(|window| {
                                    Retained::as_ptr(&window.ns_window) as usize != window_id
                                });
                            }),
                        );
